...
| `detect_non_global_regex_sanitizer`                | `/vuln/non-global-sanitizer`     | regex `.replace()` sanitizer missing the `/g` flag          |
| `detect_prototype_pollution_merge_gadget`          | `/vuln/prototype-pollution`      | `Object.assign()` merges a `JSON.parse()` result            |
